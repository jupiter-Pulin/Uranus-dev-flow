...
1. 無迭代計數器 — 可能無限循環（`rules/auto-loop.md:83` 定義了 3-round exit 但無持久化）
...
|-----------|--------|-----------|
| Finding key: `file + canonical_issue_text` | `review-common.md:154` | Direct (fingerprint base) |
...
**Rule migration**: `max_rounds` 預設 10，取代 `rules/auto-loop.md:83` 的 "3 rounds on same issue"。實施時需同步更新 `auto-loop.md` exit condition 為引用 state file `max_rounds`，並在 `auto-loop-project.md` 開放 per-project override。
...
| AC | Status | Evidence |
|----|--------|----------|
| AC-1: description | Satisfied / Partial / Violated / Not applicable | file:line |
```
...
Migration runs in **all state writers** (`post-tool-review-state.sh` `init_state_file()` AND `post-edit-format.sh` state reset). Extract shared function:

```bash
# Shared migration function (sourced or inlined in both hooks)
_migrate_state_v2() {
...
**All state writers**: `post-tool-review-state.sh` (init_state_file), `post-edit-format.sh` (state reset on file edit). Both call `_migrate_state_v2` before any write.
...
- [ ] Nit history 是否應加入 `.gitignore`（personal vs team shared）？
