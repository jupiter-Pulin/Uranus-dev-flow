...
- `AskUserQuestion` — already in `allowed-tools` for create-request
...
sequenceDiagram
    ...
    S->>G: Phase 1.5: Assess granularity
    G->>G: Count ACs, check layers, check scope
    alt Signals ≥ 2
    ...
    S->>S: Phase 2-4: Explore → Generate → Confirm
```

### 3.2 Phase 1.5: Granularity Check

Insert between Phase 1 (Gather) and Phase 2 (Explore) in SKILL.md.

#### 3.2.1 Signal Detection

| Signal | Detection Method | Weight |
|--------|-----------------|--------|
| **AC count > 8** | Count `- [ ]` items in gathered requirements/AC list. Exclude recognized boilerplate quality gates (`Pass /codex-review-doc`, `Pass /precommit-fast`) from count. | Primary |
| **Layer mixing** | Check if Related Files or Requirements span different concern layers: behavior-layer (`rules/`, `CLAUDE.md`, `commands/*.md`, `skills/**/*.md`) vs code-layer (`hooks/`, `scripts/`, `*.sh`, `*.js`) | Primary |
...
When `signal_count >= 1.5`, use AskUserQuestion:
...
1. Generate N sibling request files with naming: `YYYY-MM-DD-{feature}-r{N}.md`
2. Each sibling gets:
...
Quality-gate ACs (like "Pass /codex-review-doc") don't count toward the ≤8 target.
```

#### 3.3.2 Add `Depends On` Field

Add to template after `## References`:

```markdown
## Dependencies

- Depends On: [Request Title](./YYYY-MM-DD-xxx.md) ← only if this request requires another to be completed first
```
...
```
Phase 1: Gather → Phase 1.5: Granularity Check → Phase 2: Explore → Phase 3: Generate → Phase 4: Confirm
```
...
|-----------|--------|------|
| `AskUserQuestion` in allowed-tools | Already available | None |
...
| Unit | Split naming — `YYYY-MM-DD-{feature}-r{N}.md` format | String test |
...
|---|---------|---------------|-------|
| 1 | Should quality-gate ACs be identified by pattern or by explicit list? | UX decision | Pattern: `/codex-review-doc`, `/precommit-fast`, `/codex-review-fast` |
