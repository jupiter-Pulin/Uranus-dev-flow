...
const commandPath = resolve(root, 'commands/test-health.md');
...
test('commands/test-health.md exists', () => {
  assert.ok(existsSync(commandPath), 'commands/test-health.md should exist');
});

test('commands/test-health.md has --full and --collect flags', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /--full/, 'should have --full flag');
  assert.match(content, /--collect/, 'should have --collect flag');
});
