...
const commandPath = resolve(root, 'commands/debug.md');
...
test('debug skill directory and files exist', () => {
  ...
  assert.ok(existsSync(commandPath), 'commands/debug.md should exist');
...
test('commands/debug.md has correct frontmatter', () => {
...
test('commands/debug.md references skill and all references', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /@skills\/debug\/SKILL\.md/, 'should reference SKILL.md');
  assert.match(content, /@skills\/debug\/references\/failure-taxonomy\.md/, 'should preload failure-taxonomy');
  assert.match(content, /@skills\/debug\/references\/probe-protocol\.md/, 'should preload probe-protocol');
  assert.match(content, /@skills\/debug\/references\/report-template\.md/, 'should preload report-template');
});

test('commands/debug.md has no-git-commit key rule', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /No git commit/, 'should have no-git-commit key rule');
});

test('commands/debug.md has --export in arguments', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /--export/, 'should document --export flag');
});
