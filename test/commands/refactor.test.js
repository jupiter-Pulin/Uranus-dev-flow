...
const commandPath = resolve(root, 'commands/refactor.md');
...
test('refactor all files exist', () => {
  const files = [skillPath, commandPath, refCatalog, refDetection, refGate, refOutput];
...
test('commands/refactor.md references SKILL.md', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /@skills\/refactor\/SKILL\.md/, 'should reference SKILL.md');
});

test('commands/refactor.md references all 4 reference files', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /refactor-catalog\.md/, 'should reference refactor-catalog');
  assert.match(content, /target-detection\.md/, 'should reference target-detection');
  assert.match(content, /behavioral-gate\.md/, 'should reference behavioral-gate');
  assert.match(content, /output-template\.md/, 'should reference output-template');
});

test('commands/refactor.md has --target and --auto arguments', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /--target/, 'should have --target argument');
  assert.match(content, /--auto/, 'should have --auto argument');
});
