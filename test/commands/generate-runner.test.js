...
const cmdPath = resolve(__dirname, '../../commands/generate-runner.md');
...
test('command.md has matching allowed-tools', () => {
  const skillTools = skillContent.match(/allowed-tools:\s*(.+)/)?.[1]?.trim();
  const cmdTools = cmdContent.match(/allowed-tools:\s*(.+)/)?.[1]?.trim();
  assert.equal(skillTools, cmdTools);
});
...
test('command.md references SKILL.md and templates', () => {
  assert.ok(cmdContent.includes('@skills/generate-runner/SKILL.md'));
  assert.ok(cmdContent.includes('@skills/generate-runner/references/templates.md'));
});
