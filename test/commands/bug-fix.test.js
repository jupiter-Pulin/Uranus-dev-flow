...
const commandPath = resolve(root, 'commands/bug-fix.md');
...
// --- commands/bug-fix.md assertions ---

test('commands/bug-fix.md has correct allowed-tools (Bash, not scoped)', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /allowed-tools:.*Read, Grep, Glob, Edit, Write, Bash/, 'should have full tool list');
  assert.doesNotMatch(content, /Bash\(git/, 'should not have scoped Bash(git:*)');
});

test('commands/bug-fix.md has /codex-test-review in workflow', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /codex-test-review/, 'should include test review in workflow');
});

test('commands/bug-fix.md does not reference testing-guide.md', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.doesNotMatch(content, /testing-guide\.md/, 'should not reference deleted testing-guide.md');
});

test('commands/bug-fix.md has no-git-commit key rule', () => {
  const content = readFileSync(commandPath, 'utf8');
