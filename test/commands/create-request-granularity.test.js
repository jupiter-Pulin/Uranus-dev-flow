...
test('command spec has granularity check in workflow', () => {
  const content = readFileSync(resolve(root, 'commands/create-request.md'), 'utf8');
  assert.match(content, /Granularity Check/, 'should mention Granularity Check');
  assert.match(content, /1\.5a/, 'should have Phase 1.5a');
  assert.match(content, /1\.5b/, 'should have Phase 1.5b');
});

test('command spec has AskUserQuestion in allowed-tools', () => {
  const content = readFileSync(resolve(root, 'commands/create-request.md'), 'utf8');
  assert.match(content, /AskUserQuestion/, 'command should have AskUserQuestion');
});
