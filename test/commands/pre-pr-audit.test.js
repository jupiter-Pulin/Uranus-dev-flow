...
test('commands/pre-pr-audit.md exists with correct frontmatter', () => {
  const path = resolve(root, 'commands/pre-pr-audit.md');
  assert.ok(existsSync(path), 'command file should exist');
  const content = readFileSync(path, 'utf8');
  assert.match(content, /description:/, 'should have description frontmatter');
  assert.match(content, /allowed-tools:/, 'should have allowed-tools');
  assert.match(content, /@skills\/pre-pr-audit\/SKILL\.md/, 'should reference skill');
});
