...
const commandPath = resolve(root, 'commands/jira.md');
...
test('S4: Command file references all skill references', () => {
  assert.ok(existsSync(commandPath), 'commands/jira.md missing');
  ...
  for (const ref of refFiles) {
    const directive = `@skills/jira/references/${ref}`;
    assert.ok(content.includes(directive), `Command missing reference: ${directive}`);
  }

  assert.ok(
    content.includes('@skills/jira/SKILL.md'),
    'Command missing @skills/jira/SKILL.md'
  );
});
