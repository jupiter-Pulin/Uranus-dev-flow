...
const commandPath = resolve(root, 'commands/test-deep.md');
...
test('commands/test-deep.md has --all and --layer flags', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /--all/, 'should have --all argument');
  assert.match(content, /--layer/, 'should have --layer argument');
});

test('commands/test-deep.md has --no-fail-fast flag', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /--no-fail-fast/, 'should have --no-fail-fast argument');
});

// --- CLAUDE.md assertion ---

test('CLAUDE.md has /test-deep entry', () => {
  const content = readFileSync(resolve(root, 'CLAUDE.md'), 'utf8');
  assert.match(content, /test-deep/, 'should have test-deep in command reference');
});
