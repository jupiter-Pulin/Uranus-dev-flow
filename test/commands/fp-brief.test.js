...
const commandPath = resolve(root, 'commands/fp-brief.md');
...
test('commands/fp-brief.md exists', () => {
  assert.ok(existsSync(commandPath), 'commands/fp-brief.md should exist');
});

test('commands/fp-brief.md has required flags', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /--depth/, 'should have --depth flag');
  assert.match(content, /--verify/, 'should have --verify flag');
  assert.match(content, /--output/, 'should have --output flag');
  assert.match(content, /--no-save/, 'should have --no-save flag');
});

test('commands/fp-brief.md references SKILL.md', () => {
  const content = readFileSync(commandPath, 'utf8');
  assert.match(content, /@skills\/fp-brief\/SKILL\.md/, 'should reference skill');
});
